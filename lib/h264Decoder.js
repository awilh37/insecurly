...
class H264Decoder {
  constructor(canvas) {
    ...
    this.frameBuffer = [];
    this.firstKeyframeReceived = false; // Track first IDR frame
    this.stats = {
      framesDecoded: 0,
      framesDropped: 0,
      totalBytes: 0,
  ...
  decodeRTPPacket(packet) {
    ...
    // WebCodecs requires a keyframe (IDR) first
    // If no keyframes received yet, treat first frame as keyframe
    if (this.useWebCodecs && this.videoDecoder && !this.firstKeyframeReceived) {
      if (!packet.isKeyframe) {
        // Force first frame as keyframe to initialize decoder
        if (this.stats.framesDecoded === 0) {
          console.log('[H264] First frame - forcing as keyframe for decoder initialization');
          packet.isKeyframe = true;
        } else {
          console.log('[H264] Waiting for keyframe...');
          this.stats.framesDropped++;
          return;
        }
      }
      this.firstKeyframeReceived = true;
      console.log('[H264] First frame received, decoder ready');
    }
    ...
    try {
      ...
      if (this.useWebCodecs && this.videoDecoder) {
        // WebCodecs path: convert RTP to EncodedVideoChunk
        this._decodeWithWebCodecs(packet, decodingStart);
      } else {
        // Fallback: buffer frame for canvas rendering
